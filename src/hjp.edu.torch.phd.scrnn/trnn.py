...
parser.add_argument('--data',dest='data',
                    help='path to data folder containing {train|dev|test}.txt, default=./data',default="/home/hjp/Downloads/data")
parser.add_argument('--size-hidden',dest='n_hidden', type=int, help='hidden size', default=10)
parser.add_argument('--model',dest='model', help='model type rnn|lstm', default='rnn')
parser.add_argument('--timeout',dest='timeout',type=int, help='timeout in seconds', default=600)
...
class TreeRNN(nn.Module):
  def __init__(self, word_vocab, hdim, nc):
    ...
    self.WO = nn.Linear(hdim, nc)        # nc: number of class.
  ...
  def expr_for_tree(self, tree, decorate=False):
    ...
    expr = self.TANH(self.WR(torch.cat((e1, e2),1)))        # concat e1 and e2
    if decorate:
      tree._e = expr
    return expr
