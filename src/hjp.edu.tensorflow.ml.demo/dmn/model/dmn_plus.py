...
from model.dmn_plus_epi import EpisodeModule
from util.nn import weight, bias, dropout, batch_norm
...
class DMN(BaseModel):
    def build(self):
        ...
        with tf.name_scope('SentenceReader'):
            ...
            input_embed = tf.transpose(tf.pack(input_embed), [2, 1, 0, 3])
            encoded = l * input_embed * input_mask
            facts = tf.reduce_sum(encoded, 2)
        ...
        with tf.name_scope('InputFusion'):
            ...
            facts = forward_states + backward_states
    ...
    def preprocess_batch(self, batches):
        ...
        for n in range(N):
            for i, sentence in enumerate(input[n]):
                ...
                input_masks[n, i, :sentence_len, :] = 1.
