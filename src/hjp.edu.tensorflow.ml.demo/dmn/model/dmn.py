...
from model.dmn_epi import EpisodeModule
from util.nn import weight, batch_norm, dropout
...
class DMN(BaseModel):
    def build(self):
        ...
        input = tf.placeholder(tf.float32, shape=[N, L, V], name='x')
        question = tf.placeholder(tf.float32, shape=[N, Q, V], name='q')
        answer = tf.placeholder(tf.int64, shape=[N], name='y')
        input_mask = tf.placeholder(tf.bool, shape=[N, L], name='x_mask')
        ...
        with tf.variable_scope('input') as scope:
            ...
            question_vec = questions[-1]

        input_states = tf.transpose(tf.pack(input_states), [1, 0, 2])
        facts = []
        for n in range(N):
            filtered = tf.boolean_mask(input_states[n, :, :], input_mask[n, :])
            padding = tf.zeros(tf.pack([F - tf.shape(filtered)[0], d]))
            facts.append(tf.concat(0, [filtered, padding]))

        facked = tf.pack(facts)
        facts = tf.unpack(tf.transpose(facked, [1, 0, 2]), num=F)
    ...
    def make_decoder_batch_input(self, input):
        input_transposed = tf.transpose(input, [1, 0, 2])
